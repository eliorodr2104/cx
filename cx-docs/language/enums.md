...
an initializer, an assignment, a `return`, an argument, a tuple element of known type, the
other operand of `==` or `!=`, and a `case` label of a Cx switch. A bare `ok` is an error.
