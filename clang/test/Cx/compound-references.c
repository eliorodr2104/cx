...
// CHECK-DAG: @as_int = global ptr @"_Z17_Cx0$Draw$move$x:i"
void (*as_float)(float) = &move(x:);
// CHECK-DAG: @as_float = global ptr @"_Z17_Cx0$Draw$move$x:f"

// A single match needs no further selection.
void (*only_y)(int) = &move(y:);
// CHECK-DAG: @only_y = global ptr @"_Z17_Cx0$Draw$move$y:i"

// `_` names a position that has no label.
void (*unlabeled)(int, int) = &plain(_:_:);
// CHECK-DAG: @unlabeled = global ptr @"_Z15_Cx0$Draw$plainii"
